...
@implementation RTOpenSSLPrivateKey
...
-(NSData *) decryptData:(NSData *)cipherText error:(NSError **)error
{
  ...
  if (!ctx) {
    RT_RETURN_OPENSSL_ERROR(ContextAllocFailed, nil);
  }
  
  NSData *result = ^NSData *{
    
    if (EVP_PKEY_decrypt_init(ctx) <= 0) {
      RT_RETURN_OPENSSL_ERROR(DecryptInitFailed, nil);
    }
    
    if (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_OAEP_PADDING) <= 0) {
      RT_RETURN_OPENSSL_ERROR(DecryptPaddingFailed, nil);
    }
    
    size_t clearTextLen = 0;
    if (EVP_PKEY_decrypt(ctx, NULL, &clearTextLen, cipherText.bytes, cipherText.length) <= 0) {
      RT_RETURN_OPENSSL_ERROR(DecryptFailed, nil);
    }
...
-(NSData *) signData:(NSData *)data withPadding:(RTDigitalSignaturePadding)padding error:(NSError **)error
{
  ...
  NSData *sig = ^NSData *{
    ...
    if (EVP_PKEY_sign_init(ctx) <= 0) {
      RT_RETURN_OPENSSL_ERROR(SignInitFailed, nil);
    }
    
    switch (padding) {
      case RTDigitalSignaturePaddingPKCS1:
        if (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING) <= 0) {
          RT_RETURN_OPENSSL_ERROR(PaddingFailed, nil);
        }
        break;
        
      case RTDigitalSignaturePaddingPSS32:
        if (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PSS_PADDING) <= 0) {
          RT_RETURN_OPENSSL_ERROR(PaddingFailed, nil);
        }
        
        if (EVP_PKEY_CTX_set_rsa_pss_saltlen(ctx, 32) <= 0) {
          RT_RETURN_OPENSSL_ERROR(SaltLengthFailed, nil);
        }
        break;
    }
    
    if (EVP_PKEY_CTX_set_signature_md(ctx, EVP_sha256()) <= 0) {
      RT_RETURN_OPENSSL_ERROR(SignatureFailed, nil);
    }
    
    size_t sigLen = 0;
    if (EVP_PKEY_sign(ctx, NULL, &sigLen, digest.bytes, digest.length) <= 0) {
      RT_RETURN_OPENSSL_ERROR(SignFailed, nil);
    }
...
-(NSData *) exportPKCS12Named:(NSString *)name withPassphrase:(NSString *)passphrase error:(NSError **)error
{
  ...
  if (pkcs12 == NULL) {
    RT_RETURN_OPENSSL_ERROR(PKCS12ExportFailed, nil);
  }
...
@implementation RTOpenSSLPublicKey
...
-(NSData *) encryptData:(NSData *)clearText error:(NSError **)error
{
  ...
  NSData *result = ^NSData *{
    ...
    if (EVP_PKEY_encrypt_init(ctx) <= 0) {
      RT_RETURN_OPENSSL_ERROR(EncryptInitFailed, nil);
    }
    
    if (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_OAEP_PADDING) <= 0) {
      RT_RETURN_OPENSSL_ERROR(EncryptPaddingFailed, nil);
    }
    
    size_t cipherTextLen = 0;
    if (EVP_PKEY_encrypt(ctx, NULL, &cipherTextLen, clearText.bytes, clearText.length) <= 0) {
      RT_RETURN_OPENSSL_ERROR(EncryptFailed, nil);
    }
...
-(BOOL) verifyData:(NSData *)data againstSignature:(NSData *)signature withPadding:(RTDigitalSignaturePadding)padding result:(BOOL *)result error:(NSError **)error
{
  NSData *digest = [data sha256];
  
  EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new(_pointer, NULL);
  if (!ctx) {
    RT_RETURN_OPENSSL_ERROR(ContextAllocFailed, NO);
  }
  
  BOOL ret = ^BOOL {
    
    if (EVP_PKEY_verify_init(ctx) <= 0) {
      RT_RETURN_OPENSSL_ERROR(VerifyInitFailed, NO);
    }
    
    switch (padding) {
      case RTDigitalSignaturePaddingPKCS1:
        if (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING) <= 0) {
          RT_RETURN_OPENSSL_ERROR(PaddingFailed, NO);
        }
        break;
      ...
      case RTDigitalSignaturePaddingPSS32:
        ...
        if (EVP_PKEY_CTX_set_rsa_pss_saltlen(ctx, 32) <= 0) {
          RT_RETURN_OPENSSL_ERROR(SaltLengthFailed, NO);
        }
        break;
    }
    
    if (EVP_PKEY_CTX_set_signature_md(ctx, EVP_sha256()) <= 0) {
      RT_RETURN_OPENSSL_ERROR(SignatureFailed, NO);
    }
...
@implementation RTOpenSSLKeyPair
...
{
  EVP_PKEY *key = [RTAsymmetricKeyPairGenerator generateRSAKeyPairWithKeySize:keySize error:error];
  if (!key) {
    return nil;
  }

  RTOpenSSLKeyPair* pair = [[RTOpenSSLKeyPair alloc] initWithKeyPointer:key];
...
+(instancetype) importPKCS12:(NSData *)pkcs12Data withPassphrase:(NSString *)passphrase error:(NSError **)error
{
  ...
  RTOpenSSLKeyPair *pair = [[self alloc] initWithKeyPointer:privateKey];
...
-(instancetype) initWithKeyPointer:(EVP_PKEY *)pointer
{
  ...
  if (self) {
    ...
    _privateKey = [[RTOpenSSLPrivateKey alloc] initWithKeyPointer:pointer];
    _publicKey = [[RTOpenSSLPublicKey alloc] initWithKeyPointer:pointer];
...
-(instancetype) initWithCoder:(NSCoder *)aDecoder
{
  ...
  if (self) {
    ...
    _publicKey = [[RTOpenSSLPublicKey alloc] initWithKeyPointer:_privateKey.pointer];
  }
  return self;
}
