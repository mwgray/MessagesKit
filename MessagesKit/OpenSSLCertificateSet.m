...
@interface OpenSSLCertificateSet ()
...
@implementation OpenSSLCertificateSet
...
-(OpenSSLCertificate *)objectAtIndexedSubscript:(NSUInteger)idx
{
  ...
  return [[OpenSSLCertificate alloc] initWithCertPointer:cert];
}
...
-(NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state objects:(__unsafe_unretained id  _Nonnull *)buffer count:(NSUInteger)len
{
  ...
  for (localIdx=0, totalIdx=state->state; localIdx < len && totalIdx < sk_X509_num(_pointer); localIdx++, totalIdx++) {
    ...
    OpenSSLCertificate *__autoreleasing cert =
      [[OpenSSLCertificate alloc] initWithCertPointer:sk_X509_value(_pointer, (int)totalIdx)];
