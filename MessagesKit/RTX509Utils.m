                                       ...
                                       error:(NSError **)error
{
  ...
  if (cert == NULL) {
    ...
    RT_RETURN_OPENSSL_ERROR(CertBuildFailed, NULL);
  }
  
  BOOL certValid = ^BOOL {
    
    { // Set Issuer Name field
      X509_NAME* issuerName = [RTX509Utils nameWithDictionary:@{@"CN":@"reTXT"}];
      if (!X509_set_issuer_name(cert, issuerName)) {
        X509_NAME_free(issuerName);
        RT_RETURN_OPENSSL_ERROR(CertBuildFailed, NO);
      }
      X509_NAME_free(issuerName);
    }
    
    { // Set Subject Name field
      X509_NAME* subjectName = [RTX509Utils nameWithDictionary:name];
    ...
    [RTX509Utils addExtenstionNamed:SN_key_usage
                          withValue:keyUsage
                      toCertificate:cert];
